export default defineEventHandler(async (event) => {
    ...
    if (!query.fromUnit) throw createError({ statusCode: 400, message: "`fromUnit` is not defined" });
    if (!query.toUnit) throw createError({ statusCode: 400, message: "`toUnit` is not defined" });
    ...
    if (typeof response === "string") throw createError({ statusCode: 400, message: response });
